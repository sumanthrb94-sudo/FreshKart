 ...
 *
 * `stalled` swaps the spinner for a retry. The splash is shown while the
 * signed-in user's profile is being read, and that read can fail for as long
 * as the network is bad — a spinner that has already been spinning for half a
 * minute is lying about being busy, and leaves nothing to press.
 */
      ...
      <div className="relative flex h-[100dvh] w-full max-w-app flex-col items-center justify-center px-10 text-center">
        ...
        {showRetry ? (
          <button
            type="button"
            onClick={onRetry}
            className="mt-7 rounded-xl bg-brand-500 px-6 py-3 text-sm font-bold text-white transition-colors hover:bg-brand-600"
          >
            Try again
          </button>
        ) : (
