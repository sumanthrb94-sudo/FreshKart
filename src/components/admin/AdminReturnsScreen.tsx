...
/**
 * Returns — what a buyer handed back at the door.
 *
 * There is no post-delivery return in this business: the buyer inspects the
 * delivery while the driver is still there and refuses what they don't want,
 * so a "return" here is always a deduction made at the doorstep, never a
 * pickup to arrange. Small ones the driver settles himself; anything over his
 * limit lands in this queue with a driver standing in a shop waiting for the
 * answer, which is why it leads the screen and updates live.
 */
export function AdminReturnsScreen() {
  ...
  const settled = live.filter((o) => o.adjustment && o.adjustment.status !== "PENDING");
  ...
  async function decide(order: Order, decision: "APPROVED" | "REJECTED") {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Couldn't settle the return.");
    } finally {
      setBusyId(null);
    }
  }
  ...
  return (
    <AdminShell>
      <div className="flex flex-col gap-3 p-4">
        <h1 className="text-lg font-extrabold text-fg">Returns</h1>
        <p className="-mt-1 text-xs text-fg-subtle">
          Goods handed back at the door. The buyer is billed only for what they kept either way —
          approving writes the stock off, rejecting puts it back in inventory.
        </p>
